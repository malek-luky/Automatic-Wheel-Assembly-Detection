...
def test_config_loads():
    ...
    assert hparams.batch_size > 0, "Hyper parameter 'batch_size' from default_config.yaml is corrupted. Expected value \
        should be greater than 0."
    assert (
        hparams.hidden_layer_size > 0
    ), "Hyper parameter 'hidden_layer_size' from default_config.yaml is corrupted. Expected value \
        should be greater than 0."
    assert (
        hparams.output_size == 1
    ), "Hyper parameter 'output_size' from default_config.yaml is corrupted. Expected value \
        should be 1."
    assert (
        hparams.sequence_length > 0
    ), "Hyper parameter 'sequence_length' from default_config.yaml is corrupted. Expected value \
        should be greater than 0."
